...
* 1.10 时间值
UNIX 系统使用过两种时间值：
1. 日历时间： UTC 格林尼治标准时间，系统基本数据类型 time_t 用于保存这种时间值。
2. 进程时间： 被称为CPU时间，用以度量进程使用的中央处理器资源，进程时间以时钟滴答计算。每秒曾经取为 50 60 100 个时钟滴答。系统基本类型clock_t 保存这种时间值。 2.4.5节将说明如何用sysconf函数得到每秒的时钟滴答数。
当度量一个进程执行时间，UNIX系统为一个进程维护了3个进程时间值： 时钟时间、用户CPU时间、系统CPU时间
